...
export default {
  ...
  methods: {
    ...
    handle: throttle(async function (type, disNum, cart) {
      // type：为了区分这三个元素
      // 目前的disNum形参：+ 变化量(1) , - 变化量(-1)  input 最终的个数（并不是变化量）
      ...
      switch (type) {
        ...
        case "minus":
          ...
          break;
        case "change":
          // 用户输入进来的最终量是一个非法的（带有英文汉字）
          if (isNaN(disNum) || disNum < 1) {
            disNum = 1;
          } else {
            parseInt(disNum) - cart.skuNum;
          }
          if (disNum < cart.skuNum) {
            disNum = disNum - cart.skuNum;
            // console.log('我执行了',disNum,cart.skuNum);
          }
          break;
      }
      // 派发action
      ...
      // console.log("带给服务器变化量的数字", disNum,cart.skuNum);
    }, 500),
    // 删除某一个产品的操作
