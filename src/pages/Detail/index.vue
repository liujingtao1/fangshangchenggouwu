        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            ...
            <div class="cartWrap">
              ...
              <div class="add">
                <!-- 以前的路由跳转，从A路由跳转到B路由，这里在加入购物车，进行路由跳转之前，发请求
...
export default {
  ...
  methods: {
    ...
    // 表单元素修改产品个数
    changSkuNumber(event) {
      // 用户输入进来的文本 * 1
      let value = event.target.value * 1;
      // 如果用户输入进来的不是number格式，出现NaN或者小于1的数字
    ...
    async addShopCart() {
      /* 派发action 
        ...
        派发成功之后，还需要判断是否加入购物车成功还是失败，进行相应的操作
      ...
      */
      try {
        ...
        // 产品信息的数据【比较复杂：比如skuInfo】，我们一般通过会话存储（会话结束数据就会消失）
        // 本地存储|会话存储，一般存储的是字符串
        sessionStorage.setItem("SKUINFO",JSON.stringify(this.skuInfo))
        this.$router.push({name: 'addcartsuccess',query:{skuNumber:this.skuNumber}})
        /*
          浏览器存储功能：HTML5新增的，本地存储（local Storage）和会话存储（session Storage）；
